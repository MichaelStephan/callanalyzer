...
(def no-requestid-exception
  {:type :no-request-id :message "request-id not found"})
(def multiple-request-ids-exception
  {:type :multiple-request-ids :message "multiple request-ids found"})

(defn make-es-conn [endpoint]
  (info "Creating elastichsearch pool connection")
  (esr/connect
    endpoint {:conn-manager (clj-http.conn-mgr/make-reusable-conn-manager {:timeout 30000})}))
...
(defn get-vcap-request-ids [logs]
  (post-process-ids (map c/get-vcap-request-id logs)))
...
(defmethod search :request-id [{:keys [value]}]
  ...
  (let [query {:term {:requestid value}}]
    (es-search query)))
...
(defmethod search :vcap-request-ids [{:keys [value]}]
  ...
  (if (c/not-empty? value)
    (let [query {:bool {:should
                        (flatten (map #(vector {:term {:vcaprequestid %}}
...
(defmethod search :default [{:keys [field value]}]
...
(defmethod search-with-deps :default [{:keys [field value]}]
  (warn "Searching for" field "not supported"))
...
; b2249518-ebbd-471e-71fe-e9f961a09eb8
