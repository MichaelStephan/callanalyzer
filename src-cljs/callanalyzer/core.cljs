...
(defn ^:export search [option value results]
  (chsk-send! [@option @value] (* 3 60 1000) #(swap! results conj %))
  (reset! value ""))
...
(defn ui-search-button [option value results]
  [:div
   [:input {:type "button"
            :value "Search"
            :on-click #(search option value results)}]])
...
(defn ui-search [results]
  (let [option (r/atom :search/request-id)
    ...
    [:div 
      ...
      [ui-search-button option value results]
...
(defn ui-rtr [i]
  ^{:key (gensym)} [:li (let [hop (:hop i) s (:_source i) m (:message s)]
                          (str (clojure.string/join "" (repeat hop "-")) "> " (:client i) " calls " (:service i) (:request m) " (" (:response m) "/" (* 1000.0 (:response_time m)) ")"))
                    (ui-app i)])
